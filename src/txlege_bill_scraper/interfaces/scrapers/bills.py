...
class BillDetailScraper(DetailScrapingInterface):
    ...
    @classmethod
    async def get_basic_details(cls, client: httpx.AsyncClient, bill: TXLegeBill) -> TXLegeBill:
        try:
            ...
            bill.subjects = [s.strip() for s in subjects.text.split('<br>') if s.strip()] if subjects else []
                ...
                if companion_link:
                    ...
                    bill.companions.append(
                        BillCompanion(
                            ...
                            companion_bill_number=companion_link.text.replace(" ", "").strip(),
            ...
            bill.committees = {}
            if house_committee:
                bill.committees.update(house_committee)
            if senate_committee:
                bill.committees.update(senate_committee)
            return bill
    ...
    @classmethod
    async def parse_bill_text_table(cls, client: httpx.AsyncClient, bill: TXLegeBill) -> Dict[str, BillVersion]:
        ...
        while retries < max_retries:
            try:
                ...
                retries += 1
                logfire.warn(f"Failed to connect to {text_tab_url}, retrying {retries}/{max_retries}")
                await asyncio.sleep(2)  # Optional: wait before retrying
        else:
            logfire.error(f"Failed to connect to {text_tab_url} after {max_retries} attempts")
            return versions
        try:
            soup = BeautifulSoup(response.text, 'html.parser')
    ...
    @classmethod
    async def parse_amendments_table(cls, client: httpx.AsyncClient, bill: TXLegeBill) -> List[BillAmendment]:
        amendments = []
        amendments_tab_url = (
        ...
        while retries < max_retries:
            try:
                ...
                retries += 1
                logfire.warn(f"Failed to connect to {amendments_tab_url}, retrying {retries}/{max_retries}")
                await asyncio.sleep(2)
        else:
            logfire.error(f"Failed to connect to {amendments_tab_url} after {max_retries} attempts")
            return bill
            ...
            if len(cells) >= 7:
                ...
                if _coauthors_link:
                    ...
                    if co_tables:
                        ...
                        amendment_info.co_authors = [x.strip() for x in co_rows.text.split('|')]
                else:
    ...
    async def fetch(
            ...
            _sem: asyncio.Semaphore) -> Dict[str, TXLegeBill]:
        counter = 0
        async def get_individual_bill(bill: TXLegeBill) -> Dict[str, TXLegeBill]:
            async with _sem:
                try:
                    ...
                    bill.versions = await cls.parse_bill_text_table(client=_client, bill=bill)
                    bill.amendments = await cls.parse_amendments_table(client=_client, bill=bill)
                    bill.create_ids()
                    nonlocal counter
                    counter += 1
                    if counter % 250 == 0:
